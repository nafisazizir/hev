"""Vendored kev model code so that released kev checkpoints can be scored under kev's exact packing.

Source: github.com/jaredpalmer/kev, file kev/model.py at commit
20fa6268c8ceb226530be2fb5266ab2c36b37724 (Apache-2.0, Copyright Jared Palmer; see THIRD_PARTY_NOTICES.md).

Why this exists: hev.api and hev.data.materialize render records byte-identically to kev, but hev.model.encode
packs them differently (Hev puts <decide> before the options, shares option start positions and always isolates
options; kev puts <decide> after the options with sequential positions). A kev checkpoint therefore has to be run
through kev's own encoder, mask, PointerHead and DecisionModel, which are reproduced below. hev.kev.KevPredictor
wraps them behind the same duck-typed contract as hev.evaluate.LocalPredictor.

The released jaredpalmer/kev-0.6b was trained at kev commit 6cfa03ab. Between 6cfa03ab and 20fa626 kev/model.py
gained head_dim, option_isolation, special_embeddings, lora_targets and dtype knobs; with their defaults
(256 / False / False / "all" / fp32), which is what an old head.pt resolves to, encode, the mask and the forward
pass are unchanged, so scoring under HEAD reproduces the training-time model.

DO NOT EDIT anything below the marker line except to re-vendor from upstream. Everything after the marker is the
upstream file verbatim: tests/test_kev.py hashes it against KEV_MODEL_SHA256 (and, when the sibling kev clone is
present, against `git show <KEV_COMMIT>:kev/model.py`). Tests inject a fake backbone by monkeypatching the
module-level name `AutoModelForCausalLM`, which requires no seam in the vendored code.
"""

KEV_COMMIT = "20fa6268c8ceb226530be2fb5266ab2c36b37724"
KEV_MODEL_SHA256 = "839dd7632ec291ac4740035c86f780f8f13da2b74b54a146ce90ff0ce8de527b"
VENDOR_MARKER = "# ---- verbatim kev/model.py below this line ----\n"

# ---- verbatim kev/model.py below this line ----
