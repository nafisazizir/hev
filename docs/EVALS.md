...
- **Training partition.** `decision-v4/train.jsonl` (10,896 records / 13,896 questions per the manifest, sha256 `cb55b79e…`) is not in the repo; `hev.suite` fetches it on demand from kev's pinned Hub mirror `jaredpalmer/kev-suites` and verifies it against the manifest before use (opt-in `fetch=True`; `.gitignore` keeps it out of git). Hev trained on it in M4b: three predeclared seeds of PointerHead under kev's published v4 recipe, guarded by `hev.train --recipe kev-v4`, which refuses to start unless every shared knob matches kev's trial. The suite's own `trainable_sources` / `eval_only_sources` decide which arms may train (`hev.data.source_policy`), so `legacy_policy` and `compositional` train while `contrastive` never does. [Seeds](../runs/m4b-eval-v4-s0/result.json), [aggregate](../runs/m4b-v4-three-seed/result.json)
